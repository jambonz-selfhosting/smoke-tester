...
// booleans only. The SP policy is exercised by the verbs test
// (TestMediaCapture_NoRecordHeader), which runs alone in its package; this
// package runs concurrently with it, so it never changes the SP policy.
//
// Steps:
...
//  5. reject-non-boolean — bad disable_media_capture / support_audio_capture_policy returns 400
func TestMediaCapture_Account_REST(t *testing.T) {
	...
	s.Logf("service provider policy=%s, account derived=%s",
		sp.SupportAudioCapturePolicy, acct.ServiceProviderAudioCapturePolicy)
	s.Done()
	...
	for _, bad := range []any{"disable", true, nil} {
		body := map[string]any{"support_audio_capture_policy": bad}
		_, err = spClient.Request(ctx, http.MethodPut, "/ServiceProviders/"+cfg.SPSID, body, "", http.StatusNoContent)
		if got := provision.StatusOf(err); got != http.StatusBadRequest {
			s.Errorf("SP PUT support_audio_capture_policy=%#v: status %d, want 400 (err=%v)", bad, got, err)
		}
	}
	s.Done()
}
