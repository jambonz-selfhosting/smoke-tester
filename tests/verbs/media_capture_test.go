...
// flips the suite account flag and the SP policy (restored in Cleanup).
package verbs
...
// TestMediaCapture_NoRecordHeader — for each SP policy × account flag, place one
// inbound and one outbound call and assert the header is present exactly when
// capture resolves to off (disable_all, or per_account with the account opted out).
//
// Steps:
//  1. read-initial-flags — remember the SP's current policy for cleanup
//  2. provision-application — webhook Application the UAC dials
//  3. set-flags — PUT account flag (account scope) + PUT SP policy (SP scope)
//  4. assert-derived-field — GET /Accounts/{sid} reflects both values
...
//
// Test     --PUT disable_media_capture / policy-->         api-server
...
func TestMediaCapture_NoRecordHeader(t *testing.T) {
	...
	s.Logf("service provider support_audio_capture_policy=%s", spInitial)
	t.Cleanup(func() {
	...
	cases := []struct {
		...
		{"per_account", false, false},
		{"per_account", true, true},
		{"allow_all", false, false},
		{"allow_all", true, false},
		{"disable_all", false, true},
		{"disable_all", true, true},
	}
	for _, tc := range cases {
		...
		t.Run(fmt.Sprintf("%s_account_%s", tc.policy, onOff(tc.acc)), func(t *testing.T) {
			...
			if acct.ServiceProviderAudioCapturePolicy != tc.policy {
				s.Errorf("service_provider_audio_capture_policy: got %q want %q",
					acct.ServiceProviderAudioCapturePolicy, tc.policy)
			}
			s.Done()
