...
| 3.5a | `dial` end-of-verb teardown (WS app) | `call_hook` (wss), `dial.actionHook` over the socket | ☑ | ☐ | local | `TestVerb_Dial_WS_NoActionHook_EndsCall`, `_ActionHookNoAck_EndsCall` and `_ActionHookEmpty_EndsCall` (`tests/verbs/dial_no_action_hook_test.go`). A WebSocket application parks in `CallSession._awaitCommandsOrHangup()` when the verb list empties, which is wrong after a `dial` the app was never asked about: no `actionHook`; an `actionHook` whose ack never came (the only way a hook can fail over WS — `Session.ScriptActionHookNoAck`); or one acked with `[]`, which deliberately overrides the ws "ack now, send verbs later" pattern for `dial`. Both left the caller in dead air until `timeLimit`; fixed in feature-server (`TaskDial._endSessionUnlessHandedOff` → `CallSession.expectNoFurtherVerbs`, skipped when the app already handed the call off via LCC-replace or a completed REFER). Assertion is the caller's BYE within a budget measured from the B-leg BYE — measured 0.5s (no hook) and 5.5s (5s ack timeout) against a 90s `timeLimit`, so a regression cannot be masked by the max-duration timer. **The actionHook must be a RELATIVE path**: `ws-requestor.js` short-circuits an absolute `http(s)` hook to a plain HTTP webhook, which never reaches the app socket. |
...
| 4.15 | `amd` (on `dial`) | `amd.actionHook` | ☑ | ☑ | `@jambonz/schema` (`callbacks/amd`) | AMD runs on the dialed leg; media server is **mediajam** (Go). **All four green on jambonz.me:** `TestVerb_Dial_AMD_HumanDetected` (`amd_human_detected`, reason "short greeting", +`amd_stopped`), `_MachineDetected` (`amd_machine_detected` reason "long greeting", +`amd_machine_stopped_speaking`), `_NoSpeechDetected` (`amd_no_speech_detected`+`amd_stopped`), `_DecisionTimeout` (`amd_decision_timeout`+`amd_stopped`, forced by `noSpeechTimeoutMs≫decisionTimeoutMs`+silence). **Bug found & fixed via this test:** human/machine detection initially never fired — the FreeSWITCH→mediajam migration renamed transcription events to `stt.transcription` (gather/transcribe/listen migrated via `sttEvents()`), but `amd-utils.js` still registered the legacy `deepgram_transcribe::transcription` listener that mediajam never emits, so AMD saw no transcripts (only its `setTimeout` timers fired). Fixed in feature-server `lib/utils/amd-utils.js` (route AMD's listener registration through `sttEvents()`). `amd_stopped` is not asserted on the machine path (deferred: AMD keeps avmd running for a beep post-machine). **Skip-stubs:** `amd_tone_detected` — mediajam has a Go mod_avmd port (`internal/audiofx/avmd.go`), but a clean 1000Hz sine over G.711 µ-law didn't trip it (needs a codec-robust beep fixture); `amd_error` — separate feature-server bug: `amd-utils.js` emits it on event name `amd_error` while `dial.js` only listens on `amd`, so it never reaches the actionHook. |
...
|---|---|---|---|---|---|---|
| 5.0 | `agent` | DEEPSEEK_API_KEY (inline auth) + Deepgram (in-jambonz cred + offline STT) | ☑ | ☑ | local + upstream | 12 tests cover round-trip echo, eventHook (`user_transcript`/`llm_response`/`turn_end`), `greeting:true`, `actionHook` on end (call_sid + completion_reason + customerData), `toolHook` round-trip (function call → JSON body reply → LLM speak), a second `toolHook` test proving argument extraction (`TestVerb_Agent_ToolHook_Arguments`: `get_weather(location)` populated from real user speech, e.g. "What is the weather in Chicago?" → `arguments.location=="Chicago"`, corroborated by `turn_end.tool_calls`) — so tool tests now cover both parameterless AND argument-populated function calls, `bargeIn` + `user_interruption`, `noResponseTimeout` re-prompt, `turnDetection:"krisp"`, `noiseIsolation` 3 variants (krisp/rnnoise/object). Per-test routing for hooks without customerData via `?X-Test-Id=` query param. Drift: latency.{stt_ms,eot_ms,llm_ms,tts_ms,tool_ms} + turn_end.{confidence,tool_calls} added to `schemas/callbacks/agent-turn.schema.json` with TODO upstream. |
| 5.1 | `llm` | vendor API key (Deepgram Voice Agent used for toolHook coverage) | ☑ | ◐ | local | `TestVerb_LLM_Deepgram_ToolHook` (`tests/verbs/llm_test.go`) — first tool-calling coverage for the `llm` verb: declares `get_weather` under `Settings.agent.think.functions`, wires a verb-level `toolHook`, asserts the Voice Agent emits a function call (`args.location=="Chicago"`) and that a dynamic `FunctionCallResponse` envelope (returned via new `Session.ScriptActionHookBodyFunc`, needed to echo the live `tool_call_id`) round-trips to a spoken reply. Contract: local `schemas/callbacks/llm-tool.schema.json` (requires `args`), log-only validation, `TODO: upstream`. **Drift**: the `agent` verb's toolHook payload uses field `arguments` (row 5.0) while the `llm` verb's toolHook payload uses field `args` — candidate upstream doc/schema clarification. `eventHook`/`actionHook` for `llm` remain untested. |
| 5.2 | `lex` | AWS access key + secret | ☐ | ☐ | local | Lex V2 session; intents + transcription. |
| 5.3 | `dialogflow` (CX) | Google service-account JSON + `DIALOGFLOW_AGENT` | ☑ | ☐ | local | `TestVerb_Dialogflow_CX` (`tests/verbs/dialogflow_test.go`) — three-turn client-side tool-call round trip against a CX Playbook agent: `tool_calls` event → `toolHook` → `{outputParameters}` → `dialogflow_cx_tool_result` primes the next turn. Turn-1 (`getGeolocation`) assertions strict; turn-2 (`getFlights`) soft, since the playbook LLM occasionally skips the tool. Welcome event / intent event / actionHook-on-end still untested. |
| 5.3a | `dialogflow` (CES) | Google service-account JSON + `DIALOGFLOW_CES_APP` | ☐ | ☐ | local | `TestVerb_Dialogflow_CES` (`tests/verbs/dialogflow_ces_test.go`) — regression test for the CES tool-call stall (mediajam emitted no `tool_calls` event for CES and had no tool-result send path, so a client-side tool was never answered and the turn hung). Proves the mid-stream path: `tool_calls` → `toolHook` → `dialogflow_ces_tool_result` → `dialogflow.toolResult` → `SessionInput.tool_responses` on the open `BidiRunSession`. Strict assertions: a toolHook callback with a correlatable `tool_call.id`, populated `tool_call.args`, an eventHook `tool-calls` delivery, and — the load-bearing one — the agent must speak OUR canned tool output back (expected tokens derived from it, >=2 required), which is what distinguishes "received the result" from "hallucinated a plausible reply". Verified by mutation. Prompts + canned output are hardcoded fixtures (as in the CX test); only the app coordinates are env-supplied. **Blocked on a provisioned CES app** — a CES app is a different GCP resource from a CX agent, so `DIALOGFLOW_AGENT` cannot stand in; skips until `DIALOGFLOW_CES_APP` is set. Also requires mediajam + `@jambonz/mrf` + feature-server deployed together. |
| 5.3b | `dialogflow` (ES) | Google service-account JSON | ☐ | ☐ | local | Untested. |
