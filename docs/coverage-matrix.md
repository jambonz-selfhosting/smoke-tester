...
| 3.5a | `dial` end-of-verb teardown (WS app) | `call_hook` (wss), `dial.actionHook` over the socket | ☑ | ☐ | local | `TestVerb_Dial_WS_NoActionHook_EndsCall` + `TestVerb_Dial_WS_ActionHookNoAck_EndsCall` (`tests/verbs/dial_no_action_hook_test.go`). A WebSocket application parks in `CallSession._awaitCommandsOrHangup()` when the verb list empties, which is wrong after a `dial` the app was never asked about: no `actionHook`, or an `actionHook` whose ack never came (the only way a hook can fail over WS — `Session.ScriptActionHookNoAck`). Both left the caller in dead air until `timeLimit`; fixed in feature-server (`TaskDial._endSessionUnlessHandedOff` → `CallSession.expectNoFurtherVerbs`, skipped when the app already handed the call off via LCC-replace or a completed REFER). Assertion is the caller's BYE within a budget measured from the B-leg BYE — measured 0.5s (no hook) and 5.5s (5s ack timeout) against a 90s `timeLimit`, so a regression cannot be masked by the max-duration timer. **The actionHook must be a RELATIVE path**: `ws-requestor.js` short-circuits an absolute `http(s)` hook to a plain HTTP webhook, which never reaches the app socket. |
